...
static void cleanupGlobalData(AJGlobalData*);
static bool fillBufferWithContentsOfFile(const UString& fileName, Vector<char>& buffer);

static AJValue JSC_HOST_CALL functionPrint(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionDebug(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionGC(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionVersion(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionRun(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionLoad(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionCheckSyntax(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionReadline(ExecState*, AJObject*, AJValue, const ArgList&);
static NO_RETURN_WITH_VALUE AJValue JSC_HOST_CALL functionQuit(ExecState*, AJObject*, AJValue, const ArgList&);

#if ENABLE(SAMPLING_FLAGS)
static AJValue JSC_HOST_CALL functionSetSamplingFlags(ExecState*, AJObject*, AJValue, const ArgList&);
static AJValue JSC_HOST_CALL functionClearSamplingFlags(ExecState*, AJObject*, AJValue, const ArgList&);
#endif
...
class GlobalObject : public AJGlobalObject {
public:
    ...
    : AJGlobalObject()
{
    ...
    AJObject* array = constructEmptyArray(globalExec());
...
AJValue JSC_HOST_CALL functionDebug(ExecState* exec, AJObject*, AJValue, const ArgList& args)
{
    fprintf(stderr, "--> %s\n", args.at(0).toString(exec).UTF8String().data());
    return jsUndefined();
}

AJValue JSC_HOST_CALL functionGC(ExecState* exec, AJObject*, AJValue, const ArgList&)
{
    AJLock lock(SilenceAssertionsOnly);
    exec->heap()->collectAllGarbage();
    return jsUndefined();
}

AJValue JSC_HOST_CALL functionVersion(ExecState*, AJObject*, AJValue, const ArgList&)
{
...
AJValue JSC_HOST_CALL functionLoad(ExecState* exec, AJObject* o, AJValue v, const ArgList& args)
{
    UNUSED_PARAM(o);
    UNUSED_PARAM(v);
    UString fileName = args.at(0).toString(exec);
...
#if ENABLE(SAMPLING_FLAGS)
AJValue JSC_HOST_CALL functionSetSamplingFlags(ExecState* exec, AJObject*, AJValue, const ArgList& args)
{
    for (unsigned i = 0; i < args.size(); ++i) {
...
AJValue JSC_HOST_CALL functionClearSamplingFlags(ExecState* exec, AJObject*, AJValue, const ArgList& args)
{
    for (unsigned i = 0; i < args.size(); ++i) {
        unsigned flag = static_cast<unsigned>(args.at(i).toNumber(exec));
        if ((flag >= 1) && (flag <= 32))
...
AJValue JSC_HOST_CALL functionReadline(ExecState* exec, AJObject*, AJValue, const ArgList&)
{
...
AJValue JSC_HOST_CALL functionQuit(ExecState* exec, AJObject*, AJValue, const ArgList&)
{
...
int jscmain(int argc, char** argv, AJGlobalData*);
...
int main(int argc, char** argv)
{
    ...
    // Initialize JSC before getting AJGlobalData.
    AJ::initializeThreading();

    // We can't use destructors in the following code because it uses Windows
    // Structured Exception Handling
    int res = 0;
    AJGlobalData* globalData = AJGlobalData::create(ThreadStackTypeLarge).releaseRef();
    TRY
...
static void cleanupGlobalData(AJGlobalData* globalData)
{
    AJLock lock(SilenceAssertionsOnly);
...
static bool runWithScripts(GlobalObject* globalObject, const Vector<Script>& scripts, bool dump)
{
    ...
    AJGlobalData* globalData = globalObject->globalData();
...
static NO_RETURN void printUsageStatement(AJGlobalData* globalData, bool help = false)
{
...
static void parseArguments(int argc, char** argv, Options& options, AJGlobalData* globalData)
{
    int i = 1;
    for (; i < argc; ++i) {
...
int jscmain(int argc, char** argv, AJGlobalData* globalData)
{
    AJLock lock(SilenceAssertionsOnly);
