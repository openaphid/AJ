...
pthread_key_t AJLockCount;

static void createAJLockCount()
{
    pthread_key_create(&AJLockCount, 0);
}

pthread_once_t createAJLockCountOnce = PTHREAD_ONCE_INIT;

// Lock nesting count.
intptr_t AJLock::lockCount()
{
    pthread_once(&createAJLockCountOnce, createAJLockCount);

    return reinterpret_cast<intptr_t>(pthread_getspecific(AJLockCount));
}

static void setLockCount(intptr_t count)
{
    ASSERT(count >= 0);
    pthread_setspecific(AJLockCount, reinterpret_cast<void*>(count));
}

AJLock::AJLock(ExecState* exec)
    : m_lockBehavior(exec->globalData().isSharedInstance() ? LockForReal : SilenceAssertionsOnly)
{
    lock(m_lockBehavior);
}

void AJLock::lock(AJLockBehavior lockBehavior)
{
#ifdef NDEBUG
...
void AJLock::unlock(AJLockBehavior lockBehavior)
{
    ASSERT(lockCount());
...
AJLock::DropAllLocks::DropAllLocks(ExecState* exec)
    : m_lockBehavior(exec->globalData().isSharedInstance() ? LockForReal : SilenceAssertionsOnly)
{
    pthread_once(&createAJLockCountOnce, createAJLockCount);
    ...
    m_lockCount = AJLock::lockCount();
    for (intptr_t i = 0; i < m_lockCount; i++)
        AJLock::unlock(m_lockBehavior);
}

AJLock::DropAllLocks::DropAllLocks(AJLockBehavior AJLockBehavior)
    : m_lockBehavior(AJLockBehavior)
{
    pthread_once(&createAJLockCountOnce, createAJLockCount);
...
AJLock::DropAllLocks::~DropAllLocks()
{
    for (intptr_t i = 0; i < m_lockCount; i++)
        AJLock::lock(m_lockBehavior);
...
void AJLock::lock(AJLockBehavior)
{
}

void AJLock::unlock(AJLockBehavior)
{
}

void AJLock::lock(ExecState*)
{
}

void AJLock::unlock(ExecState*)
{
}

AJLock::DropAllLocks::DropAllLocks(ExecState*)
{
}

AJLock::DropAllLocks::DropAllLocks(AJLockBehavior)
{
}
