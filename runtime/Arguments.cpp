...
void Arguments::markChildren(MarkStack& markStack)
{
    ...
    if (d->registerArray)
        markStack.appendValues(reinterpret_cast<AJValue*>(d->registerArray.get()), d->numParameters);

    if (d->extraArguments) {
        unsigned numExtraArguments = d->numArguments - d->numParameters;
        markStack.appendValues(reinterpret_cast<AJValue*>(d->extraArguments), numExtraArguments);
    }
...
bool Arguments::getOwnPropertySlot(ExecState* exec, unsigned i, PropertySlot& slot)
{
    ...
    return AJObject::getOwnPropertySlot(exec, Identifier(exec, UString::from(i)), slot);
}
...
void Arguments::put(ExecState* exec, unsigned i, AJValue value, PutPropertySlot& slot)
{
    if (i < d->numArguments && (!d->deletedArguments || !d->deletedArguments[i])) {
        if (i < d->numParameters)
            d->registers[d->firstParameterIndex + i] = AJValue(value);
        else
            d->extraArguments[i - d->numParameters] = AJValue(value);
        return;
    }

    AJObject::put(exec, Identifier(exec, UString::from(i)), value, slot);
}

void Arguments::put(ExecState* exec, const Identifier& propertyName, AJValue value, PutPropertySlot& slot)
{
    bool isArrayIndex;
    unsigned i = propertyName.toArrayIndex(&isArrayIndex);
    if (isArrayIndex && i < d->numArguments && (!d->deletedArguments || !d->deletedArguments[i])) {
...
bool Arguments::deleteProperty(ExecState* exec, unsigned i) 
{
    ...
    return AJObject::deleteProperty(exec, Identifier(exec, UString::from(i)));
}
