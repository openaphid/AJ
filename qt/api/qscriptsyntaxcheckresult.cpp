...
{
    if (m_exception)
        AJValueUnprotect(m_engine->context(), m_exception);
}
...
QString QScriptSyntaxCheckResultPrivate::errorMessage() const
{
    ...
    AJStringRef tmp = AJValueToStringCopy(m_engine->context(), m_exception, /* exception */ 0);
...
int QScriptSyntaxCheckResultPrivate::errorLineNumber() const
{
    ...
    AJStringRef lineAttrName = QScriptConverter::toString("line");
    AJValueRef line = AJObjectGetProperty(m_engine->context(),
                                          m_exception,
                                          lineAttrName,
                                          /* exceptions */0);
    AJStringRelease(lineAttrName);
    return AJValueToNumber(m_engine->context(), line, /* exceptions */0);
}
