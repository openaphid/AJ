...
Heap::Heap(AJGlobalData* globalData)
    : m_markListSet(0)
#if ENABLE(JSC_MULTIPLE_THREADS)
...
void Heap::destroy()
{
    ...
    RefPtr<AJGlobalData> protect(m_globalData);
...
#error Need to initialize pagesize safely.
    ...
    for (size_t i = 0; i < HeapConstants::cellsPerBlock; ++i)
        new (block->cells + i) AJCell(dummyMarkableCellStructure);
    ...
    do {
        ...
        do {
            ...
            if (!block->marked.get(m_heap.nextCell)) { // Always false for the last cell in the block
                ...
                AJCell* imp = reinterpret_cast<AJCell*>(cell);
                imp->~AJCell();
        ...
        if (isPossibleCell(x)) {
            ...
            for (size_t block = 0; block < usedBlocks; block++) {
                if (blocks[block] != blockAddr)
                    continue;
                markStack.append(reinterpret_cast<AJCell*>(xAsBits));
                markStack.drain();
            }
        }
    }
...
#error Unknown Architecture
    ...
    if (result != KERN_SUCCESS) {
        WTFReportFatalError(__FILE__, __LINE__, WTF_PRETTY_FUNCTION, 
                            "AJ garbage collection failed because thread_get_state returned an error (%d). This is probably the result of running inside Rosetta, which is not supported.", result);
        CRASH();
    }
...
void Heap::protect(AJValue k)
{
    ASSERT(k);
    ASSERT(AJLock::currentThreadIsHoldingLock() || !m_globalData->isSharedInstance());
...
void Heap::sweep()
{
    ...
    for ( ; it != end; ++it) {
        ...
        new (cell) AJCell(dummyMarkableCellStructure);
#endif
    }
...
{
    size_t count = 0;
    if (AJGlobalObject* head = m_globalData->head) {
        AJGlobalObject* o = head;
        do {
            ++count;
            o = o->next();
        } while (o != head);
...
static const char* typeName(AJCell* cell)
{
    if (cell->isString())
        return "string";
#if USE(JSVALUE32)
