...
// ECMA 9.4
double AJValue::toInteger(ExecState* exec) const
{
    if (isInt32())
        return asInt32();
...
double AJValue::toIntegerPreserveNaN(ExecState* exec) const
{
    if (isInt32())
        return asInt32();
...
AJObject* AJValue::toObjectSlowCase(ExecState* exec) const
{
    ASSERT(!isCell());
...
AJObject* AJValue::toThisObjectSlowCase(ExecState* exec) const
{
    ASSERT(!isCell());
...
AJObject* AJValue::synthesizeObject(ExecState* exec) const
{
    ASSERT(!isCell());
    if (isNumber())
        return constructNumber(exec, asValue());
...
AJObject* AJValue::synthesizePrototype(ExecState* exec) const
{
    ASSERT(!isCell());
    if (isNumber())
...
#ifndef NDEBUG
char* AJValue::description()
{
    static const size_t size = 32;
    static char description[size];

    if (!*this)
        snprintf(description, size, "<AJValue()>");
    else if (isInt32())
