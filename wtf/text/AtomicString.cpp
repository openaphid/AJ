...
public:
    static AtomicStringTable* create()
    {
        ...
        ATFThreadData& data = wtfThreadData();
        if (pthread_main_np() || ATF::isWebThread())
...
{
    // Once possible we should make this non-lazy (constructed in ATFThreadData's constructor).
...
struct UCharBufferTranslator {
    ...
    {
        return Aphid::equal(str, buf.s, buf.length);
    }
...
struct HashAndCharactersTranslator {
    ...
    {
        return Aphid::equal(string, buffer.characters, buffer.length);
    }
